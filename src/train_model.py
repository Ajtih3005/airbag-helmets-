...
3-Class Labels (per project spec):
    0 - Normal
    1 - Near-Crash
    2 - Crash
...
Random Forest is also saved to models/random_forest.pkl for compatibility.
"""
...
import pandas as pd
import matplotlib
matplotlib.use('Agg')   # Non-interactive backend (safe for Raspberry Pi / CI)
...
def evaluate_model(model, X_train, X_test, y_train, y_test, model_name) -> dict:
    ...
    infer_time_ms = (time.perf_counter() - t1) * 1000 / len(X_test)  # ms per sample
    ...
    print(f"  Infer time/sample: {infer_time_ms:.4f} ms  (spec target: <4 ms)")
...
def plot_comparison(results: list, save_path: str = None):
    ...
    ax.set_title("Model Comparison — 3-Class Pre-Impact Rider State", color="white", fontsize=12)
...
def plot_confusion_matrix(result: dict, save_path: str = None):
    ...
    lbls = [LABEL_NAMES[i] for i in sorted(LABEL_NAMES)]
...
def plot_feature_importance(model, feature_names: list,
    ...
    ax.barh(range(top_n), top_vals[::-1], color="#4facfe", alpha=0.85, edgecolor="#0d0d0d")
...
def save_best_model(results: list, feature_names: list, save_dir: str = "../models"):
    """Pick the best model by F1 macro and save it alongside metadata."""
    ...
    print(f"  Infer ms    : {best['infer_ms']:.4f} ms/sample")
    print(f"  Saved to    : {model_path}")

    # Also save RF specifically
...
if __name__ == "__main__":
    ...
    from src.data_generator import WINDOW_SIZE, STRIDE
    ...
    plot_dir  = os.path.join(project_root, "models", "plots")
    ...
    feature_df = apply_sliding_window(df, window_size=WINDOW_SIZE, stride=STRIDE)
    ...
    plot_comparison(results, save_path=os.path.join(plot_dir, "comparison.png"))
    best = save_best_model(results, feature_cols, model_dir)
    plot_confusion_matrix(best, save_path=os.path.join(plot_dir, "confusion_matrix.png"))
    plot_feature_importance(best["model"], feature_cols,
                            title=best["model_name"],
